...
from mmrotate.models.losses.Rotated_IoU.min_enclosing_box import smallest_bounding_box
from ..builder import ROTATED_HEADS
from .rotated_anchor_head import RotatedAnchorHead
from .rotated_atss_head import RotatedATSSHead
import torch
from mmrotate.core.post_processing.bbox_nms_rotated import aug_multiclass_nms_rotated,multiclass_nms_rotated
from mmrotate.core.bbox.transforms import bbox_mapping_back,obb2hbb,obb2poly, poly2obb
from mmrotate.core.anchor import build_prior_generator,rotated_anchor_inside_flags
from mmcv.ops import box_iou_rotated
import math
@ROTATED_HEADS.register_module()
class SdlaRotatedRetinaHead(RotatedATSSHead):
    ...
    def __init__(self,
        ...
        super(SdlaRotatedRetinaHead, self).__init__(
            num_classes,
            in_channels,
    ...
    def cal_diou(self,box1:torch.Tensor, box2:torch.Tensor,version:str="le135"):
        """calculate diou

        Args:
            box1 (torch.Tensor): (n, 5)  xywhθ
            box2 (torch.Tensor): (n, 5)  xywhθ
            Returns:
            rdiou_loss (torch.Tensor): (n )
        """
        assert box1.shape == box2.shape
        n=box1.shape[0]
        iou = torch.zeros(n, device="cuda")
        for i in range(n):
            iou[i]=box_iou_rotated(box1[i].unsqueeze(0), box2[i].unsqueeze(0))
        iou = iou.squeeze(0)
        d2 = (box1[:,0] - box2[:,0]).pow(2) + (box1[:,1] - box2[:,1]).pow(2)
        corners1 = obb2poly(box1,version).reshape(n,4,2)
        corners2 = obb2poly(box2,version).reshape(n,4,2)
        corners1 = corners1.reshape(n, 4, 2).unsqueeze(0)
        corners2 = corners2.reshape(n, 4, 2).unsqueeze(0)
        w, h = smallest_bounding_box(torch.cat([corners1, corners2], dim=-2))
        w, h = w.squeeze(0), h.squeeze(0)
        c2 = w**2 + h**2
        diou = iou - d2/c2
        return diou
    ...
    def _get_targets_single(self,
        ...
        if self.assign_by_circumhbbox is not None:
            ...
            assign_result = self.assigner.assign(
                anchors, num_level_anchors_inside, gt_bboxes_assign, gt_bboxes_ignore,
        ...
        iou_weight = anchors.new_zeros(num_valid_anchors, dtype=torch.float)
        diou_weight = anchors.new_zeros(num_valid_anchors, dtype=torch.float)
        ...
        if len(pos_inds) > 0:
            ...
            #################### calculate iou_weight and diou_weight####################
            diou_weight[pos_inds] =self.cal_diou(sampling_result.pos_bboxes,sampling_result.pos_gt_bboxes)
        ...
        #use sdla
        # diou_weight[torch.isnan(diou_weight)] = 1.0
        sdla_weights = label_weights * (1.5 - diou_weight/2)  #regression weight
        if len(neg_inds) > 0:
            sdla_weights[neg_inds] = 0.0
        label_weights = label_weights * (1.5 + diou_weight/2) #classification weight
        ...
        if unmap_outputs:
            ...
            sdla_weights = unmap(sdla_weights, num_total_anchors, inside_flags)
        return (labels, label_weights, bbox_targets, bbox_weights, pos_inds,
                neg_inds, sampling_result, sdla_weights)
    def get_targets(self,
        ...
        # print("concat_anchor_list",concat_anchor_list)
        # print("gt_bboxes_list",gt_bboxes_list)
        results = multi_apply(
        ...
        pos_inds_list, neg_inds_list, sampling_results_list,all_sdla_weights) = results[:8]
        ...
        bbox_weights_list = images_to_levels(all_bbox_weights,
                                            num_level_anchors)
        sdla_weights_list = images_to_levels(all_sdla_weights,
                                                num_level_anchors)
        res = (labels_list, label_weights_list, bbox_targets_list,
            bbox_weights_list, num_total_pos, num_total_neg,sdla_weights_list)
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
                    bbox_targets, bbox_weights, sdla_weights, num_total_samples):
        ...
        sdla_weights = sdla_weights.unsqueeze(dim=2).repeat(1,1,5).reshape(-1,5)    
    ...
    def loss(self,
        ...
        num_total_pos, num_total_neg, sdla_init_weights) = cls_reg_targets
        num_total_samples = (
